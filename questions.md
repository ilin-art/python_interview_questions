...
- `tup` (кортеж) () -  упорядоченная неизменяемая последоавтельность объектов
- `dict` (словарь) {} - упорядоченная последовательность пар ключ-значение 
- `set` (множества) {} - неупорядоченная коллекция хешируемых объектов, которые не повторяются
- `frozenset` {} - неупорядоченная неизменяемая последовательность уникальных хешируемых объектов
- `bool`
- `NoneType`
...
```
my_range = range(10, 0, -2)  # создаем диапазон от 10 до 1 с шагом -2
print(list(my_range))  # вывод: [10, 8, 6, 4, 2]
```
...
В программировании термин "отображение" (mapping) часто используется для обозначения структуры данных, представляющей собой коллекцию пар "ключ-значение". Отображение позволяет связывать каждый элемент (значение) с уникальным ключом. В Python, такая структура данных представлена словарем (dict).
...
## Что происходит с запросом после ввода его в адресную строку

Запрос от ввода URL в адресную строку до обработки его Django приложением на другом сервере включает несколько этапов, которые можно разбить на следующие шаги:

1. **Ввод URL и DNS-резолюция**:
   - Пользователь вводит URL в адресную строку браузера и нажимает Enter.
   - Браузер проверяет кеш DNS (локальный кеш или кеш операционной системы) для получения IP-адреса.
   - Если в кеше нет записи, браузер отправляет запрос к DNS-серверу для резолюции доменного имени в IP-адрес.

2. **Установление TCP-соединения и SSL/TLS**:
   - После получения IP-адреса, браузер устанавливает TCP-соединение с сервером по указанному порту (обычно 80 для HTTP или 443 для HTTPS).
   - Если используется HTTPS, то после установления TCP-соединения начинается процесс SSL/TLS handshake для шифрования соединения.

3. **Отправка HTTP-запроса**:
   - После установления соединения браузер отправляет HTTP-запрос к серверу. Этот запрос содержит метод (например, GET), заголовки (headers), URL и другую информацию.

4. **Обработка запроса веб-сервером**:
   - Сервер, получив запрос, передает его веб-серверу (например, Nginx или Apache), который может быть настроен для обратного проксирования запросов к Django приложению.
   - Веб-сервер проверяет конфигурацию и определяет, что запрос должен быть передан на приложение Django.

5. **Обработка запроса в WSGI-сервере**:
   - Веб-сервер передает запрос WSGI-серверу (например, Gunicorn или uWSGI), который выполняет интерфейс между веб-сервером и приложением Django.
   - WSGI-сервер принимает запрос, создает объект WSGI-запроса и передает его Django приложению.

6. **Обработка запроса в Django приложении**:
   - Django приложение получает WSGI-запрос и передает его в middleware.
   - Middleware обрабатывает запрос, выполняет нужные операции (например, аутентификация, обработка сессий и т.д.), и затем передает запрос в диспетчер URL (URL dispatcher).
   - Диспетчер URL определяет, какое представление (view) должно обработать запрос на основании URL и вызываемых URL-шаблонов.

7. **Выполнение представления (view)**:
   - Соответствующее представление обрабатывает запрос. Оно может взаимодействовать с базой данных через модели (models), выполнять бизнес-логику и подготавливать данные для ответа.
   - Представление формирует ответ (например, HTML-страницу или JSON-данные) и возвращает его.

8. **Обратный путь: формирование HTTP-ответа**:
   - Ответ от представления проходит через middleware, которое может модифицировать или логировать его.
   - Затем WSGI-сервер получает готовый HTTP-ответ от Django приложения и передает его веб-серверу.
   - Веб-сервер отправляет HTTP-ответ обратно клиенту (браузеру).

9. **Обработка ответа браузером**:
   - Браузер получает HTTP-ответ от сервера.
   - Браузер обрабатывает ответ, отображает HTML-контент, загружает связанные ресурсы (CSS, JS, изображения и т.д.), и выполняет рендеринг страницы для пользователя.
